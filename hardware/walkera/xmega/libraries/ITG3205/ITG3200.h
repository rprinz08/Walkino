 ...
 * itg3200.h
 *
 *   Copyright (C) 2010  Ricardo Arturo Cabral 
 *   <ing dot cabral dot mejia at gmail dot com>. 
 *   All rights reserved.
 *
...
class ITG3200
{
  public:
    ...
    ITG3200(byte gyro_address = GYRO_ADDR_0);
#ifdef __AVR_XMEGA__
    void begin(xmWire *Wire);
    void begin(xmWire *Wire, byte pwr_mgm, byte fs_lpf, byte smplrt_div, byte int_cfg);
#else
    void begin();
    void begin(byte pwr_mgm, byte fs_lpf, byte smplrt_div, byte int_cfg);
#endif
    void reset();
    void sleep();
