...
#ifdef __AVR_XMEGA__
byte MMA8452Q::begin(xmWire *Wire, MMA8452Q_Scale fsr, MMA8452Q_ODR odr)
{
	this->wire = Wire;
#else
byte MMA8452Q::begin(MMA8452Q_Scale fsr, MMA8452Q_ODR odr)
{
	this->wire = &Wire;
#endif
	this->wire->begin(); // Initialize I2C

	// Haul fsr into our class variable, scale	
	scale = fsr;
...
//  accelerometers. Possible values for the fsr variable are SCALE_2G, 
//  SCALE_4G, or SCALE_8G.
...
byte MMA8452Q::readTap()
{
	...
	// Read EA bit to check if a interrupt was generated
	if (tapStat & 0x80)
		return tapStat & 0x7F;
	else
		return 0;
...
byte MMA8452Q::readRegister(MMA8452Q_Register reg)
{
	...
	this->wire->requestFrom(address, (byte) 1);
...
//	Read "len" bytes from the MMA8452Q, starting at register "reg". 
...
void MMA8452Q::readRegisters(MMA8452Q_Register reg, byte *buffer, byte len)
{
	...
	this->wire->requestFrom(address, len);

	// Hang out until we get the # of bytes we expect
	while(this->wire->available() < len)
		;

	for(int x = 0 ; x < len ; x++)
		buffer[x] = this->wire->read();    
}
