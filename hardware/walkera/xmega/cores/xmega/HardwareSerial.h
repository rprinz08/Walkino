...
class HardwareSerial : public Stream
{
    // NEVER 'private'
    protected:
        // NOTE:  xmega uses a structure 
        // pointer rather than individual 
        // pointers and the bit flags are 
        // consistent for all USART devices
...
// this is where I must include 'pins_arduino.h' 
// to get the 'USBCON' definition
#include "pins_arduino.h"

// DEFAULT SERIAL or 'SERIAL 1'
#if defined(USBCON)
    // NOTE:  'Serial1' will be the hardware 
    // serial and 'Serial' the USB serial
    // whenever 'USBCON' is defined in 
    // pins_arduino.h

    #include "USBAPI.h"
    extern HardwareSerial Serial1;
#else
    // normal hardware serial
    extern HardwareSerial Serial;
    // define as 'Serial' so compatible code 
    // won't break
    #define Serial1 Serial
#endif

// this is the same regardless of USBCON 
// (there will always be at least 2)
extern HardwareSerial Serial2;

// note these names are off by 1 with the 
// 'Serial_N_' definitions
...
// this function calls the serial event 
// handlers. You can override them 
// (hence 'weak') the default implementation 
// checks for data available and executes the 
// callback if so
